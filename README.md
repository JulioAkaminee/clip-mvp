...
consumido pela CLI, pela API HTTP e pela UI web (`clip serve`).
...
`clip serve` sobe a UI e a API (precisa dos extras: `pip install -e ".[api]"`):

| Método | Rota | Para quê |
|--------|------|----------|
| `POST` | `/api/jobs` | cria o job (`{"url": "...", "more": false, "min_score": 60}`) |
| `GET` | `/api/jobs` | lista os jobs conhecidos |
